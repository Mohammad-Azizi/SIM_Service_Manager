...
public class EtimessActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.etimess);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
			...
			@Override public Object instantiateItem(ViewGroup collection, int position) {
					...
					if (position == 0) {
							...
							recyclerview2.setLayoutManager(new LinearLayoutManager(EtimessActivity.this));
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(EtimessActivity.this));
					} else if (position == 3) {
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			n = "شبانه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("WPMeti.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			wp = "trans/WPMeti.json";
			mp = "trans/MPMeti.json";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			s = "other";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/WPMeti.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
