...
public class EtiinterActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.etiinter);
	...
	private void initializeLogic() {
		if (lan.getString("lan", "").equals("fa")) {
			try {
						               InputStream inputStream = getAssets().open("IPeti.json");
			...
			try {
						               InputStream inputStream = getAssets().open("trans/IPeti.json");
						               int size = inputStream.available();
						               byte[] buffer = new byte[size];
						               inputStream.read(buffer);
						               rjson = new String(buffer);
