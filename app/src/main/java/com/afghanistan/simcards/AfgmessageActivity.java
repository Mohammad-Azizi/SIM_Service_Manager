...
public class AfgmessageActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.afgmessage);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
			...
			@Override public Object instantiateItem(ViewGroup collection, int position) {
					...
					if (position == 0) {
							...
							recyclerview1.setLayoutManager(new LinearLayoutManager(AfgmessageActivity.this));
					} else if (position == 1) {
							...
							recyclerview2.setLayoutManager(new LinearLayoutManager(AfgmessageActivity.this));
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(AfgmessageActivity.this));
					} else if (position == 3) {
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "متفرقه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("DPCawcc.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			dp = "trans/DPCawcc.json";
			wp = "trans/WPCawcc.json";
			mp = "trans/MPCawcc.json";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			s = "Other";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/DPCawcc.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
