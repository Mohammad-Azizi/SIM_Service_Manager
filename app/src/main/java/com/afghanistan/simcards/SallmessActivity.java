...
public class SallmessActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.sallmess);
		...
		@Override public Object instantiateItem(ViewGroup collection, int position) {
			...
			if (position == 0) {
				...
				try {
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(SallmessActivity.this));
			} else if (position == 3) {
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "متفرقه";
			try {
						               InputStream inputStream = getAssets().open("MPMsalaam.json");
						               int size = inputStream.available();
						               byte[] buffer = new byte[size];
						               inputStream.read(buffer);
						               rjson = new String(buffer);
			...
			s = "Other";
			try {
						               InputStream inputStream = getAssets().open("trans/MPMsalaam.json");
						               int size = inputStream.available();
						               byte[] buffer = new byte[size];
						               inputStream.read(buffer);
						               rjson = new String(buffer);
