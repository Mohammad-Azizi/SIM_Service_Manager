...
public class Mtd1Activity extends AppCompatActivity {
	...
	public void _viewPager() {
		...
		new int[] {0xFFFFFFFF}));
		tabLayout.setTabMode(com.google.android.material.tabs.TabLayout.MODE_AUTO);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
		...
		@Override public Object instantiateItem(ViewGroup collection, int position) {
			...
			if (position == 0) {
				...
				recyclerview1.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
			} else if (position == 1) {
				...
				recyclerview2.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
				...
				recyclerview3.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
			} else if (position == 3) {
				...
				recyclerview4.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
			} else if (position == 4) {
				...
				recyclerview5.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
			}else if (position == 5) {
				...
				recyclerview6.setLayoutManager(new LinearLayoutManager(Mtd1Activity.this));
			}
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "اجتماعی";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("HPmtn.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			s = "Social";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/HPmtn.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			buttonD.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					activate.setData(Uri.parse("sms:".concat("344")));
