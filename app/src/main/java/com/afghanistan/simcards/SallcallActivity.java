...
public class SallcallActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.sallcall);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
			...
			@Override public Object instantiateItem(ViewGroup collection, int position) {
					...
					if (position == 0) {
							...
							recyclerview1.setLayoutManager(new LinearLayoutManager(SallcallActivity.this));
					} else if (position == 1) {
							...
							recyclerview2.setLayoutManager(new LinearLayoutManager(SallcallActivity.this));
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(SallcallActivity.this));
					} else if (position == 3) {
							...
							recyclerview5.setLayoutManager(new LinearLayoutManager(SallcallActivity.this));
					}
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "متفرقه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("DPCsalaam.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			dp = "trans/DPCsalaam.json";
			wp = "trans/WPCsalaam.json";
			mp = "trans/MPCsalaam.json";
			sp = "trans/OPCsalaam.json";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			s = "Other";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/DPCsalaam.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			buttonA.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					activate.setData(Uri.parse("sms:".concat("740")));
