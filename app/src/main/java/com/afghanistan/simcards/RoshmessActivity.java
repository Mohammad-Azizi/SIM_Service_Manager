...
public class RoshmessActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.roshmess);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
			...
			@Override public Object instantiateItem(ViewGroup collection, int position) {
					...
					if (position == 0) {
							...
							recyclerview1.setLayoutManager(new LinearLayoutManager(RoshmessActivity.this));
					} else if (position == 1) {
							...
							recyclerview2.setLayoutManager(new LinearLayoutManager(RoshmessActivity.this));
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(RoshmessActivity.this));
					} else if (position == 3) {
							...
							container.addView(layout5);
								recyclerview5.setLayoutManager(new LinearLayoutManager(RoshmessActivity.this));
					}
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "متفرقه ";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("DPMroshanjson");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			dp = "trans/DPMroshan.json";
			wp = "trans/WPMroshan.json";
			mp = "trans/MPMroshan.json";
			np = "trans/NPMroshan.json";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			s = "other";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/DPMroshan.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
