...
public class SalaamdatapageActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.salaamdatapage);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
		...
		@Override public Object instantiateItem(ViewGroup collection, int position) {
			...
			if (position == 0) {
				...
				recyclerview1.setLayoutManager(new LinearLayoutManager(SalaamdatapageActivity.this));
			} else if (position == 1) {
				...
				recyclerview2.setLayoutManager(new LinearLayoutManager(SalaamdatapageActivity.this));
				...
				recyclerview3.setLayoutManager(new LinearLayoutManager(SalaamdatapageActivity.this));
			} else if (position == 3) {
				...
				recyclerview4.setLayoutManager(new LinearLayoutManager(SalaamdatapageActivity.this));
			} else if (position == 4) {
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			s = "متفرقه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("DPsalaam.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			dp = "trans/DPsalaam.json";
			wp = "trans/WPsalaam.json";
			mp = "trans/MPsalaam.json";
			np = "trans/NPsalaam.json";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			s = "Other";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/DPsalaam.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
