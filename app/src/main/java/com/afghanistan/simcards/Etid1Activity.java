...
public class Etid1Activity extends AppCompatActivity {
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
		...
		@Override public Object instantiateItem(ViewGroup collection, int position) {
			...
			if (position == 0) {
				...
				recyclerview1.setLayoutManager(new LinearLayoutManager(Etid1Activity.this));
			} else if (position == 1) {
				...
				recyclerview2.setLayoutManager(new LinearLayoutManager(Etid1Activity.this));
				...
				recyclerview3.setLayoutManager(new LinearLayoutManager(Etid1Activity.this));
			} else if (position == 3) {
				...
				recyclerview4.setLayoutManager(new LinearLayoutManager(Etid1Activity.this));
			} else if (position == 4) {
				...
				recyclerview5.setLayoutManager(new LinearLayoutManager(Etid1Activity.this));
			}
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			n = "شبانه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("HPeti.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			n = "Nightly";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/HPeti.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
