...
public class MtcallActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.mtcall);
	...
	private class MyPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
			...
			@Override public Object instantiateItem(ViewGroup collection, int position) {
					...
					if (position == 0) {
							...
							recyclerview1.setLayoutManager(new LinearLayoutManager(MtcallActivity.this));
					} else if (position == 1) {
							...
							recyclerview2.setLayoutManager(new LinearLayoutManager(MtcallActivity.this));
							...
							recyclerview3.setLayoutManager(new LinearLayoutManager(MtcallActivity.this));
					} else if (position == 3) {
							...
							recyclerview4.setLayoutManager(new LinearLayoutManager(MtcallActivity.this));
					} else if (position == 4) {
	...
	public void _foraddvars() {
		if (lan.getString("lan", "").equals("fa")) {
			...
			n = "شبانه";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("HPCmtn.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
			...
			hp = "trans/HPCmtn.json";
			dp = "trans/DPCmtn.json";
			wp = "trans/WPCmtn.json";
			mp = "trans/MPCmtn.json";
			h = "Hourly";
			d = "Daily";
			w = "Weekly";
			m = "Monthly";
			n = "Nightly";
			            ...
			            try {
				               InputStream inputStream = getAssets().open("trans/HPCmtn.json");
				               int size = inputStream.available();
				               byte[] buffer = new byte[size];
				               inputStream.read(buffer);
				               rjson = new String(buffer);
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			buttonA.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					activate.setData(Uri.parse("sms:".concat("247")));
